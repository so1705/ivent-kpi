...
// 1. Upgrade getAIAdvice to the styled version and Japanese labels
const newGetAIAdvice = `const getAIAdvice = (stats, isPersonal) => {
  ...
  if (calls === 0) return (
    <div className="flex flex-col items-center justify-center py-10 opacity-50">
       ...
       <p className="font-bold">データが未蓄積です。架電を開始してください。</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-3 items-start">
        ...
        <p><span className="text-rose-400 font-bold text-[10px] uppercase block mb-0.5">分析結果</span><span className="text-white font-medium leading-relaxed">{advice.obs}</span></p>
      </div>
      <div className="flex gap-3 items-start">
...
content = content.replace(/const getAIAdvice = \(stats, isPersonal\) => \{[\s\S]+?return advice\.obs \+ "【戦略】" \+ advice\.strategy \+ " 【アクション】" \+ advice\.action;\s*\};/, newGetAIAdvice);

// 2. Restore MetricHelpModal and the data
...
content = content.replace(/\/\/ 3\. UI部品コンポーネント \(Atomic Components\)/, helpModalCode + "\n\n// 3. UI部品コンポーネント (Atomic Components)");

// 3. Update GasSyncDataView with cards and dropdowns
...
content = content.replace(/const GasSyncDataView = \(\{ gasData, members, forcedMemberId = null, hideHeader = false, onEditGasRecord, onDeleteGasRecord \}\) => \{[\s\S]+?\};/, newGasSyncView);

// 4. Update Settings Goals to include Calls and Closing Rate
const newGoalSetting = `             <h3 className="text-sm font-bold text-slate-800 border-l-4 border-indigo-600 pl-3 mt-12">全体・詳細目標設定</h3>
             <div className="p-8 bg-white border border-slate-200 space-y-6 shadow-sm rounded-3xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">週間アポ目標</label>
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">週間架電目標</label>
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase">接続時アポ率目標 (%)</label>
...
content = content.replace(/<h3 className="text-sm font-bold text-slate-800 border-l-4 border-indigo-600 pl-3 mt-12">全体目標設定<\/h3>[\s\S]+?<\/div>\s*<\/div>/, newGoalSetting);

// 5. Add showHelp state and Modal to App.jsx
...
// 6. Add "?" help buttons to Dashboard
content = content.replace(/<h3 className="text-sm font-black text-slate-800 flex items-center gap-2">\s*<div className="w-1.5 h-4 bg-slate-300 rounded-full"><\/div> 稼働効率\s*<\/h3>/, '<h3 className="text-sm font-black text-slate-800 flex items-center gap-2"><div className="w-1.5 h-4 bg-slate-300 rounded-full"></div> 稼働効率 <button onClick={()=>setShowHelp(true)} className="text-slate-300 hover:text-blue-600"><Icon p={I.Help} size={14}/></button></h3>');

fs.writeFileSync(path, content, 'utf8');
console.log("UI Restoration and Enhancement complete");
