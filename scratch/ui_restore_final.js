...
// 6. Help Modal Injection - using string search to avoid unclosed regex
const navSearch = '<nav className="fixed bottom-0';
if (!content.includes("{showHelp && <MetricHelpModal onClose={() => setShowHelp(false)} />}")) {
  content = content.split(navSearch).join("{showHelp && <MetricHelpModal onClose={() => setShowHelp(false)} />}\n      " + navSearch);
}

// 7. Help button on Dashboard
...
console.log("UI Restoration success (Final Fix)");
