      ...
      <div className="p-6 md:p-10 bg-white border border-slate-100 rounded-[2rem] md:rounded-[3rem] shadow-sm space-y-8 md:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10">
          ...
          <div className="space-y-6 p-6 bg-slate-50/50 rounded-3xl border border-slate-100">
            ...
            <button onClick={async () => {
              if (!me?.id) return alert("ログイン中のメールアドレスと紐付いているスタッフ情報が見つかりません。下の「スタッフ管理」から、ご自身のメールアドレスをスタッフ情報に登録してください。");
              ...
              <div className="flex flex-col gap-px bg-slate-200 border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
                {members.map(m => (
                  <button key={m.id} onClick={() => {
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 flex items-center gap-2">
                          {m.name}
                          {!m.email && <span className="text-[8px] bg-rose-500 text-white px-1.5 py-0.5 rounded-full animate-pulse">未連携</span>}
                        </div>
                    ...
                    <Icon p={I.Edit} size={16} className="text-slate-200 group-hover:text-slate-900 transition-colors" />
                  </button>
                ))}
              </div>
            </section>
            ...
            <div className="fixed inset-0 bg-slate-900/95 z-[300] flex items-center justify-center p-6 backdrop-blur-md">
              <div className="bg-white w-full max-w-sm p-10 space-y-8 border-4 border-slate-900 rounded-3xl shadow-2xl">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 font-bold rounded-xl" value={editingMember.email || ''} onChange={e => setEditingMember({ ...editingMember, email: e.target.value })} placeholder="Gmailアドレス" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button onClick={async () => {
                    const updated = { ...editingMember, hourlyWage: Number(editingMember.hourlyWage) };
                    await onUpdateMember(editingMember.id, updated);
                    setEditingMember(null);
                    alert("スタッフ情報を更新しました。これでログインアカウントとの紐付けが反映されます。");
                  }} className="bg-slate-900 text-white py-4 font-bold transition-all active:bg-black rounded-2xl">更新を確定</button>
                </div>
