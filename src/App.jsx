                 ...
                 <div className="space-y-12">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                     ...
                     <div className="bg-white border border-slate-100 rounded-[2.5rem] shadow-sm overflow-hidden">
                        <div className="p-5 bg-slate-50 border-b border-slate-100 text-xs font-black flex items-center justify-between text-slate-600 uppercase tracking-widest">
                           <span className="flex items-center gap-2"><Icon p={I.Users} size={16}/> メンバー別パフォーマンス</span>
                           <span className="text-[10px] text-slate-400">CPH順</span>
                        </div>
                        <div className="divide-y divide-slate-50">
                           {memberStats.map(m => (
                             <button key={m.id} onClick={()=>setDrilldownMember(m)} className="w-full p-4 bg-white hover:bg-slate-50 transition-all text-left flex items-center justify-between gap-3">
                               <div className="flex items-center gap-3 min-w-0">
                                 <div className={"w-9 h-9 rounded-xl shrink-0 ${m.role==='closer'?'bg-amber-400':'bg-blue-600'} text-white flex items-center justify-center font-black text-sm"}>{m.name.slice(0,1)}</div>
                                 <div className="min-w-0">
                                   <div className="font-bold text-slate-900 text-sm truncate">{m.name}</div>
                                   <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                     <span className="text-[10px] font-bold text-slate-400">架電 {m.calls}</span>
                                     <span className="text-[10px] text-slate-300">|</span>
                                     <span className="text-[10px] font-bold text-slate-400">接続 {m.picConnected}</span>
                                     <span className="text-[10px] text-slate-300">|</span>
                                     <span className="text-[10px] font-bold text-blue-500">アポ {m.appts}</span>
                                   </div>
                                 </div>
                               </div>
                               <div className="flex items-center gap-2 shrink-0">
                                 <span className="px-2 py-1 bg-blue-50 text-blue-600 rounded-lg font-black text-xs tabular-nums whitespace-nowrap">{m.cph} c/h</span>
                                 <div className="p-1.5 bg-slate-50 rounded-xl text-slate-300"><Icon p={I.ChevronRight} size={14}/></div>
                               </div>
                             </button>
                           ))}
                        </div>
